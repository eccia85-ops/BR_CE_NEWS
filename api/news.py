...
def gemini_summarize(articles_by_cat, range_label, brief=False):
    ...
    lines.append("제목이 달라도 같은 사건/이슈를 다룬 기사는 하나로 통합하세요.")
    lines.append("통합 시 가장 핵심적인 내용 위주로 중복 표현은 제거하세요.")
    lines.append("해당 카테고리 기사가 없으면 '해당 없음'으로 표기하세요.")
    lines.append("")
    lines.append("출력 형식 (아래 구조를 반드시 따르세요):")
    lines.append("자사 직결:")
    lines.append("- 실제 기사 내용 기반 팩트 한 줄")
    lines.append("  보령 사업 관점 시사점 한 줄")
    lines.append("시장 영향:")
    lines.append("- 실제 기사 내용 기반 팩트 한 줄")
    lines.append("  시장 영향 관점 시사점 한 줄")
    lines.append("업계 동향:")
    lines.append("- 실제 기사 내용 기반 팩트 한 줄")
    lines.append("  중장기 시사점 한 줄")
    lines.append("(기사에 없는 내용은 절대 추가하지 마세요)")
    lines.append("")
    lines.append("========================================")
    lines.append("")
