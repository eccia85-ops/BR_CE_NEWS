...
def gemini_summarize(articles_by_cat, range_label, brief=False):
    ...
    lines.append("보령제약 CE기획팀 관점에서 카테고리별로 핵심 기사를 선별하고 요약해주세요.")
    lines.append("")
    lines.append("[보령제약 회사 정보]")
    lines.append("- 주력 치료영역: 만성질환(고혈압/당뇨/이상지질/심혈관), 항암(고형암/바이오시밀러), 항생제/감염, CDMO")
    lines.append("- 주요 제품: 탁소텔(도세탁셀/항암), 온베브지(베바시주맙/항암 바이오시밀러), 알림타(pemetrexed/항암)")
    lines.append("  카나브(피마사르탄/고혈압), 트루버디/트루다파(당뇨), 젬자(gemcitabine/항암)")
    lines.append("- 주요 경쟁사: 한미약품, 대웅제약, 종근당, HK이노엔, 유한양행, 동아제약 등의 국내 제약사")
    lines.append("")
    lines.append("[기사 선별 기준]")
    lines.append("- 보령 주력 치료영역 및 제품 관련 기사 우선")
    lines.append("- 주요 경쟁사 동향은 시장영향/업계동향에서 우선 배치")
    lines.append("- 시장 구조 변화, 정책/급여/약가, 글로벌 진출, R&D/임상 주요 결과 포함")
    lines.append("- 노조/쟁의, 뷰티/화장품, 주가 현황 등 비관련 기사는 우선순위 낮음")
    lines.append("- 동일 이슈 복수 기사는 하나로 통합")
    lines.append("")
    if brief:
        lines.append("[출력 형식] 아래 형식으로만 답하세요. 마크다운 사용 금지. 간결하게:")
    else:
        lines.append("[출력 형식] 아래 형식으로만 답하세요. 마크다운 사용 금지:")
    lines.append("- 중요도 높은 항목부터 나열")
    lines.append("- 각 항목: 팩트 한 줄 / 아랫줄에 보령 관점 시사점 한 줄")
    lines.append("- 기사에 없는 내용 추가 금지")
    lines.append("- 해당 카테고리 기사가 없으면 '해당 없음'")
    lines.append("")
    lines.append("출력 구조:")
    lines.append("자사 직결:")
    lines.append("- 팩트")
